...
{
    public class ImageRightLayoutStrategy : BaseLayoutStrategy
    {
        public override void Layout(SlidePart slidePart, LiquidVictor.Entities.Slide slide)
        {
            ...
            // Add text content on the left if available
            if (textContent.Value != null)
            {
                var contentShape = CreateContentShape(textContent.Value.Title);

                // Position text content on the left side
                var contentTransform = new Transform2D();
                contentTransform.Append(new Offset() { X = 762000L, Y = 2286000L }); // ~0.75 inches from left, ~2.25 inches from top
            ...
            if (imageContent.Value != null)
            {
                ...
                pictureTransform.Append(new Offset() { X = 4572000L, Y = 2286000L }); // ~4.5 inches from left, ~2.25 inches from top
